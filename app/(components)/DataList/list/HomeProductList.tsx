...
const HomeProductList = ({ productType, tabData, listData, showTab }) => {
  ...
  useEffect(() => {
    switch (productType) {
      case PRODUCT_TYPE.MERCHANT:
        ...
        setCurList(listData.filter((item, idx) => item.mainCateg === tabData[0].categCd))
        break;
  ...
  const onClickNavItem = (categCd) => {
    ...
    setCurList(listData.filter((item, idx) => item.mainCateg === categCd));
  }
      ...
      <ul className={`table-list ${renderListClass(productType)}`}>
        {
          curList?.map((item, idx) => {
            ...
            switch (productType) {
              ...
              case PRODUCT_TYPE.POINT:
                return <PointShopItem key={item.brandId} itemData={item} />

              case PRODUCT_TYPE.MERCHANT:
                return <MerchantItem key={item.merchantId} itemData={item} />
